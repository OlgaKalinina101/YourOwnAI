...
1. **Create client** in `data/remote/[provider]/`
```kotlin
...
@Provides
@Singleton
fun provideNewProviderClient(...): NewProviderClient
```

3. **Add enum to ModelProvider.kt**
```kotlin
...
4. **Add to AIServiceImpl**
```kotlin
AIProvider.NEW_PROVIDER -> generateNewProviderResponse(...)
```

5. **Update network_security_config.xml**
```xml
<domain>api.newprovider.com</domain>
```
...
4. Add to navigation in `MainActivity.kt`
...
- **Room schema changes** - Update version or use `fallbackToDestructiveMigration`
- **ProGuard crashes** - Check `mapping/release/` for obfuscated names
- **Native crashes** - Check Llamatik threading (use Mutex)
